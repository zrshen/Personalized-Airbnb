...
function zscore(inputs, accessor) {
...
function kmeans(arrayToProcess, numberOfClusters, accessor) {
  ...
  //arrayToProcess.forEach(function(v){ delete v.Type });
  var keys = Object.keys(arrayToProcess[0]).filter(function(value, index, arr){ 
        //return value != 'Type';

        if (typeof arrayToProcess[0][value] == 'number' && value != 'id' && value != 'Latitude' && value != 'Longitude'){
                return value;}
    });
  ...
  array_zscore = [];
  keys.map(function(k){array_zscore[k] = zscore(arrayToProcess,function(d){return d[k]})});
  var matrix = [];
  for (var i=0; i<arrayToProcess.length;i++){
    matrix.push(keys.map(function(k){return array_zscore[k][i]}));
  }

  //console.log(matrix)

  var vectors = PCA.getEigenVectors(matrix);
  //var topTwo = PCA.computePercentageExplained(vectors,vectors[0],vectors[1]);
  var adData = PCA.computeAdjustedData(matrix,vectors[0],vectors[1],vectors[2],vectors[3],vectors[4],vectors[5],vectors[6],vectors[7]).formattedAdjustedData;
  for (var i=0; i<arrayToProcess.length;i++){
     arrayToProcess[i].pc1=adData[0][i];
     arrayToProcess[i].pc2=adData[1][i];
     arrayToProcess[i].pc3=adData[2][i];
     arrayToProcess[i].pc4=adData[3][i];
     arrayToProcess[i].pc5=adData[4][i];
     arrayToProcess[i].pc6=adData[5][i];
     arrayToProcess[i].pc7=adData[6][i];
     arrayToProcess[i].pc8=adData[7][i];
  };
  /*
   ...
   */
  accessor = function (d) { return [d.pc1,d.pc2,d.pc3,d.pc4,d.pc5,d.pc6,d.pc7,d.pc8]; };
  ...
  for (j = 0; j < numberOfClusters; j++) {
       for (i = 0; i < groups[j].length; i++) {
        groups[j][i].cluster = j;
        data_w_cluster.push(groups[j][i]);
       };
   };
  return data_w_cluster;
};
