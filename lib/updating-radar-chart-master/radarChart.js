function RadarChart() {
   ...
   var options = {
      ...
      // Margins for the SVG
      margins: {
         top: 50 * window.innerHeight/960,
         right: 200 * window.innerWidth/1920,
         bottom: 50 * window.innerHeight/960,
         left: 100 * window.innerWidth/1920,
      },
      ...
      legend: {
         ...
         toggle: 'circle',
         position: { x: 380 * window.innerWidth/1920, y: 100 * window.innerHeight/960 }
      },
   ...
   function chart(selection) {
        selection.each(function () {
            ...
            update = function() {
                ...
                //keys_old = _data.map(function(m) { return m.key; });

                //var key_array = new Array()

                //var keys = new Array()

                //for (i = 0; i < keys_old.length; i++) {
                //    if (legend_toggles[i] != true){
                //        //console.log(i)
                //        key_array.push(i)
                //        }
                //    }
                ...
                //    keys.push(keys_old[key_array[j]])
                //}
                ...
                if (options.legend.display) {
                   ...
                   if (d3.legend) {
                      ...
                      chart_node.selectAll('.rcRadarWrapper')
                        .attr('gen', function(d, i) {
                        //console.log(legend_toggles)
                        //console.log(legend_toggles[d])
                         if (legend_toggles[i] != true) {

                              var shape = 'opacity: 1'
                           } else {
                              var shape = 'opacity: 0.01'
                           }
                           d3.select(this).attr('style', function() { return shape; });
                           return legend_toggles[i];
   ...
   function dataCalcs() {
       ...
       _data.forEach( function(e) { e.values.forEach (function(d, i) { 
          ...
          if (ranges[axes[i]][1] != 1) {
             var range = ranges[axes[i]];
             d.original_value = Number(d.value);
             d.value = (d.value - range[0]) / (range[1] - range[0] + 0.0001);
             //console.log(d.value)
          }
