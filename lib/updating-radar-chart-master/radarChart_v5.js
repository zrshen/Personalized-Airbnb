function RadarChart() {
   ...
   function chart(selection) {
        selection.each(function () {
            ...
            update = function() {
                ...
                //var radarLine = d3.svg.lineRadial()
                var radarLine = d3.radialLine()
                   .curve( options.areas.rounded ?
                                 d3.curveCardinal:
                                 d3.curveLinear )
                ...
                var update_radarArea = update_blobWrapper.selectAll('a' + options.class + 'RadarArea')
                ...
                if (options.legend.display) {
                   var shape = d3.symbol().type(d3.symbolDiamond).size(150)();
   ...
   function dataCalcs() {
       ...
       //                 : [0, 1];
       //               //  : [e.values[0].original_value, e.values[0].original_value];                              // default
   ...
   function areaMouseover(d, i, self) {
      ...
      //Dim all blobs
      chart_node.selectAll("a" + options.class + "RadarArea")
   ...
   // on mouseover for the legend symbol
	function legendMouseover(d, i, self) {
         ...
         var area = keys.indexOf(d) >= 0 ? d : keyScale(d); 
			...
			//Bring back the hovered over blob
         chart_node.selectAll("a" + options.class + "RadarAreaa" + area.replace(/\s+/g, ''))
				.transition().duration(200)
				.style("fill-opacity", function(d, i, j) {
               return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.7;
            });	
